...
M.DEFAULT_CFG = {
   hide_hash = true, -- Print the non-array part of tables?
...
local function valid_id(cfg, x)
   ...
   if not x:match("^[a-zA-Z_][a-zA-Z0-9_]*$") then
      return false
   end
   if cfg.keywords and cfg.keywords[x] then
      return false
   end
   return true
end
...
local function __tostring(x)
   ...
   if x ~= nil then
      __tostring_cache[x] = false
   end -- nil is an illegal key
   return false
end
   ...
   if not cfg.hide_hash then
      ...
      for k, v in pairs(adt) do
         ...
         elseif type(k) == "number" and k <= alen and math.fmod(k, 1) == 0 and k > 0 then
            ...
            has_hash = true
            if valid_id(cfg, k) then
               x = x + #k
            else
               x = x + xlen(k, cfg, nested) + 2
            end -- count surrounding brackets
...
local function consider_newline(p, len)
   if not p.cfg.line_max then
      return
   end
   if p.current_offset + len <= p.cfg.line_max then
      return
   end
...
local acc_type = {
   ...
   number = function(p, adt)
      p:acc(tostring(adt))
   end,
   string = function(p, adt)
      p:acc((string.format("%q", adt):gsub("\\\n", "\\n")))
   end,
   boolean = function(p, adt)
      p:acc(adt and "true" or "false")
   end,
}
...
function acc_type.table(p, adt)
   ...
   local function indent(p)
      if not p.cfg.fix_indent then
         p.indent = p.current_offset
      else
         p.indent = p.indent + p.cfg.fix_indent
      end
   end
   ...
   if not p.cfg.hide_hash then
      for k, v in pairs(adt) do
         ...
         elseif type(k) == "number" and k <= alen and k > 0 and math.fmod(k, 1) == 0 then
            ...
            if has_hash then
               p:acc(", ")
            else -- 1st hash-part pair ever found
               p:acc("{ ")
               indent(p)
            end
            ...
            if is_id then
               expected_len = #k + xlen(v, p.cfg, p.nested) + #" = , "
            else
               expected_len = xlen(k, p.cfg, p.nested) + xlen(v, p.cfg, p.nested) + #"[] = , "
            end
   ...
   elseif has_tag and not has_hash and not has_arr then -- nothing, tag already in acc
      ...
      elseif has_tag and not has_hash and alen == 1 and type(adt[1]) ~= "table" then
         ...
         p:acc(" ")
         acc_value(p, adt[1]) -- indent= indent+(cfg.fix_indent or 0))
         no_brace = true
