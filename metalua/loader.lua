...
M.path = M.path .. ";/usr/local/lib/lua/5.1/?.lua;/usr/local/lib/lua/5.1/?/init.lua"
-- Initialize package.mpath from package.path
M.mpath = M.mpath or os.getenv("LUA_MPATH") or (M.path .. ";"):gsub("%.(lua[:;])", ".m%1"):sub(1, -2)

-- M.mpath =
--    "./?.mlua;/usr/local/share/lua/5.1/?.mlua;/usr/local/share/lua/5.1/?/init.mlua;/usr/local/lib/lua/5.1/?.mlua;/usr/local/lib/lua/5.1/?/init.mlua;/usr/share/lua/5.1/?.mlua;/usr/share/lua/5.1/?/init.mlua"
...
function M.metalua_loader(name)
   ...
   if M.mcache and pcall(require, "lfs") then
      ...
      return require("metalua.compiler").new():src_to_function(luastring, name)
   end
end
