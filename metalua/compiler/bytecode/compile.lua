...
local function ensure_list(ast)
   return ast.tag and ast.tag ~= "Do" and { ast } or ast
end
...
local function explist(fs, astlist, v, offset)
   ...
   if #astlist < offset then
      error("I don't handle empty expr lists yet")
   end
...
local function funcargs(fs, ast, v, idx_from)
   ...
   if hasmultret(args.k) then
      nparams = luaK.LUA_MULTRET
   else -- open call
      if args.k ~= "VVOID" then
         luaK:exp2nextreg(fs, args)
      end -- close last argument
...
local function new_localvar(fs, name, n)
   ...
   if fs.nactvar + n > M.MAXVARS then
      error("too many local vars")
   end
...
function stat.stat(fs, ast)
   if ast.lineinfo then
      fs.lastline = ast.lineinfo.last.line
   end
...
local function patch_goto(fs, src, dst) end
...
function stat.Label(fs, ast)
   ...
   -------------------------------------------------------------------
   if fs.labels[label_id] then
      error("Duplicate label in function")
   else
      fs.labels[label_id] = { pc = fs.pc, nactvar = fs.nactvar }
   end
...
function stat.Goto(fs, ast)
   ...
   if label then
      ...
      if fs.nactvar > label.nactvar then
         luaK:codeABC(fs, "OP_CLOSE", label.nactvar, 0, 0)
      end
...
function expr.expr(fs, ast, v)
   if type(ast) ~= "table" then
      error("Expr AST expected, got " .. pp.tostring(ast))
   end
...
function expr.Dots(fs, ast, v)
   ...
   if fs.f.is_vararg < M.VARARG_NEEDSARG then
      fs.f.is_varag = fs.f.is_vararg - M.VARARG_NEEDSARG
   end
...
function expr.Function(fs, ast, v)
   ...
   if ast.lineinfo then
      new_fs.f.lineDefined, new_fs.f.lastLineDefined = ast.lineinfo.first.line, ast.lineinfo.last.line
   end
...
function expr.Index(fs, ast, v)
   if #ast ~= 2 then
      print("\n\nBAD INDEX AST:")
      pp.print(ast)
      error("generalized indexes not implemented")
   end
