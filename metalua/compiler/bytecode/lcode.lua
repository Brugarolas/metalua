...
function luaK:_nil(fs, from, n)
   if fs.pc > fs.lasttarget then -- no jumps to current position?
      if fs.pc == 0 then
         return
      end --function start, positions are already clean
...
function luaK:need_value(fs, list, cond)
   while list ~= self.NO_JUMP do
      ...
      if luaP:GET_OPCODE(i) ~= "OP_TESTSET" or luaP:GETARG_A(i) ~= luaP.NO_REG or luaP:GETARG_C(i) ~= cond then
         return true
      end
...
function luaK:patchtestreg(fs, node, reg)
   ...
   if luaP:GET_OPCODE(i) ~= "OP_TESTSET" then
      return false
   end -- cannot patch other instructions
...
function luaK:freereg(fs, reg)
   if not luaP:ISK(reg) and reg >= fs.nactvar then
      ...
      assert(reg == fs.freereg, string.format("reg=%i, fs.freereg=%i", reg, fs.freereg))
   end
end
   ...
   if k == "VNIL" or k == "VTRUE" or k == "VFALSE" or k == "VKNUM" then
      if fs.nk <= luaP.MAXINDEXRK then
         if k == "VNIL" then
            e.info = self:nilK(fs)
         elseif k == "VKNUM" then
            e.info = self:numberK(fs, e.nval)
         else
            e.info = self:boolK(fs, e.k == "VTRUE")
         end
         e.k = "VK"
...
function luaK:invertjump(fs, e)
   ...
   assert(
      luaP:testOpMode(luaP:GET_OPCODE(pc), "OpModeT")
...
function luaK:constfolding(op, e1, e2)
   ...
   elseif op == "OP_LEN" then
      ...
      assert(false, "Unknown numeric value")
   end
   e1.nval = r
   return true
end
...
function luaK:prefix(fs, op, e)
   local e2 = { t = self.NO_JUMP, f = self.NO_JUMP, k = "VKNUM", nval = 0 }
   if op == "unm" then
      if e.k == "VK" then
         self:exp2anyreg(fs, e)
      end
