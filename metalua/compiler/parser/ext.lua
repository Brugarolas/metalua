...
return function(M)
   ...
   M.lambda_expr = gg.sequence({
      ...
      builder = function(x)
         ...
         return { tag = "Function", x[1], { { tag = "Return", x[2], lineinfo = li }, lineinfo = li } }
      end,
   })
   ...
   function M.expr_in_backquotes(lx)
      return M.expr(lx, 35)
   end -- 35=limited precedence
   M.expr.infix:add({
      name = "infix function",
      "`",
      _M.expr_in_backquotes,
      "`",
      prec = 35,
      assoc = "left",
   ...
   local function op_assign(kw, op)
      ...
      local function f(a, b)
         if #a ~= #b then
            gg.parse_error("assymetric operator+assignment")
         end
         local right = {}
         local r = { tag = "Set", a, right }
         for i = 1, #a do
            right[i] = { tag = "Op", op, a[i], b[i] }
         end
         return r
      end
      M.lexer:add(kw)
