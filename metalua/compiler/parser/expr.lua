...
return function(M)
   ...
   M.func_params_content = gg.list({
      ...
      gg.multisequence({ { "...", builder = "Dots" }, _M.id }),
      separators = ",",
      terminators = { ")", "|" },
   })
   ...
   function M.id_or_literal(lx)
      ...
      if a.tag ~= "Id" and a.tag ~= "String" and a.tag ~= "Number" then
         ...
         elseif a.tag == "Keyword" then
            msg = "An expression was expected, and `" .. a[1] .. "' can't start an expression"
         else
   ...
   local function opf2r(op)
      return function(a, _, b)
         return { tag = "Op", op, b, a }
      end
   end
   ...
   local function op_ne(a, _, b)
      ...
      return {
         ...
         "not",
         {
            ...
            b,
            lineinfo = {
               first = a.lineinfo.first,
               last = b.lineinfo.last,
            },
         },
      }
   end
   ...
   M.expr = gg.expr({
      ...
      infix = {
         ...
         -- { "&", prec = 50, builder = opf2("band") },
         -- { "|", prec = 10, builder = opf2("bor") },
         -- { "~", prec = 40, builder = opf2("bxor") },
         -- { "<<", prec = 60, builder = opf2("shl") },
         -- { ">>", prec = 60, builder = opf2("shr") },
      },
      ...
      suffix = {
         name = "expr suffix op",
         {
            ...
            _M.expr,
            "]",
            builder = function(tab, idx)
               return { tag = "Index", tab, idx[1] }
            end,
         },
         {
            ".",
            _M.id,
            builder = function(tab, field)
               return { tag = "Index", tab, _M.id2string(field[1]) }
            end,
         },
         {
            "(",
            _M.func_args_content,
            ")",
            builder = function(f, args)
               return { tag = "Call", f, unpack(args[1]) }
            end,
         },
         {
            "{",
            _table.content,
            "}",
            builder = function(f, arg)
               return { tag = "Call", f, arg[1] }
            end,
         },
         {
            ":",
            _M.id,
            _M.method_args,
            builder = function(obj, post)
               local m_name, args = unpack(post)
               return { tag = "Invoke", obj, _M.id2string(m_name), unpack(args) }
            end,
         },
         {
            "+{",
