...
return function(M)
   ...
   ----------------------------------------------------------------------------
   local method_name = gg.onkeyword({
      name = "method invocation",
      ":",
      _M.id,
      transformers = {
         function(x)
            return x and x.tag == "Id" and M.id2string(x)
         end,
      },
   })
      ...
      if op then
         ...
         elseif e[1].tag ~= "Call" and e[1].tag ~= "Invoke" then
            ...
            elseif e[1].tag == "Op" then
               ...
               typename = "is of type '" .. (e[1].tag or "<list>") .. "'"
            end
            gg.parse_error(
               lx,
               "This expression %s; "
   ...
   M.local_stat_parser = gg.multisequence({
      ...
      default = gg.sequence({
         ...
         builder = function(x)
            local left, right = unpack(x)
            return { tag = "Local", left, right or {} }
         end,
      }),
   })
   ...
   M.stat = gg.multisequence({
      ...
      { "if", gg.nonempty(elseifs_parser), gg.onkeyword({ "else", M.block }), "end", builder = if_builder },
   ...
   function M.assignments:add(k, v)
      self[k] = v
   end
