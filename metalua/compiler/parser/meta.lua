...
-- Compile-time metaprogramming features: splicing ASTs generated during compilation, AST quasi-quoting helpers.
...
return function(M)
   ...
   function M.meta.eval(ast)
      ...
      local f
      -- if jit then
      --    local generate = require("metalua.jit.compiler")
      --    f = loadstring(generate(ast))
      -- else
      f = mlc:ast_to_function(ast, "=splice")
      -- end
   ...
   local function lift(ast)
      ...
      function cases.number(t)
         return { tag = "Number", t, quote = true }
      end
      function cases.string(t)
         return { tag = "String", t, quote = true }
      end
      function cases.boolean(t)
         return { tag = t and "True" or "False", t, quote = true }
      end
      local f = cases[type(ast)]
      if f then
         return f(ast)
      else
         error("Cannot quote an AST containing " .. tostring(ast))
      end
   end
   M.meta.lift = lift
